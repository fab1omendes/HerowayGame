...
function createBoard() {
  ...
  function createHero(options) {
    ...
    document.addEventListener('keydown', (event) => {
      console.log('keydown was pressed', event.key)
      hero.move(event.key)
    })
  }
  ...
  function createEnemy(options) {
    ...
    setInterval(() => {
      ...
      const ramdomIndex = Math.floor(Math.random() * direction.length);
      const randomDirection = direction[ramdomIndex]
...
//item que vai ser renderizado (item)-> mini-demon | chest | hero | trap
// top - posição X - number
// left -posição Y - number

for (var i=2; i<18; i=i+3){
  board.createItem({ item: 'trap', top: TILE_SIZE * i, left: TILE_SIZE * i})
}
...
for (var i=2; i<18; i++){
  board.createEnemy({ top: TILE_SIZE * i, left: TILE_SIZE * i })
}
